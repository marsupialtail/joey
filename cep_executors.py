...
class CEPExecutor(Executor):
    ...
    def execute(self,batches,stream_id, executor_id):
        ...
        # startt2 = time.time()
        for bound in tqdm(polars.from_arrow(result).to_dicts()):
            ...
            while stack:
                ...
                startt = time.time()
                query = "select {}, {} from frame where {}".format(self.row_count_col, self.time_col, next_event_filter)
                matched = polars.SQLContext(frame=remaining_df).execute(query).collect()
                # print(query, len(remaining_df))
...
crimes = polars.read_parquet("crimes.parquet").with_row_count().to_arrow()
