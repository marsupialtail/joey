            ...
            for seq_len in sorted(this_row_can_be)[::-1]:
                ...
                if matched_sequences[seq_len - 1] is not None and len(matched_sequences[seq_len - 1]) > 0:
                    ...
                    if stubs[seq_len] is None:
                        ...
                        # print(predicate)
                        l = []
                        for j, col in enumerate(event_independent_columns[event_names[seq_len]]):
                            col_name = event_names[seq_len] + "_" + col
                            if col_name in predicate:
                                predicate = predicate.replace(event_names[seq_len] + "_" + col, "$" + str( len(l)+ 1))
                                l.append(col)
                        stubs[seq_len] = l
                        prepare_stmt = "prepare v{} as select * from x where {}".format(seq_len, predicate)
                        # print(prepare_stmt)
