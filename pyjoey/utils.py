...
def verify(data, results, conditions):
    ...
    for condition in conditions:
        ...
        events.append(
            data[event_nrs].rename(
                {col: event_name + "_" + col for col in data[event_nrs].columns}
            )
        )
    ...
    all_predicate = (
        sqlglot.parse_one(all_predicate)
        .transform(
            lambda node: sqlglot.exp.column(node.table + "_" + node.name)
    ...
    result = polars.from_arrow(
        con.execute(
            "select count(*) from events_arrow where {}".format(all_predicate)
        ).arrow()
    )["count_star()"][0]
    ...
    if not result == len(results):
        polars.from_arrow(
            con.execute(
                "select * from events_arrow where not ({})".format(all_predicate)
            ).arrow()
        ).write_parquet("problem.parquet")
    ...
    for event_name, condition in conditions:
        result = result.union(
            set(
                [
                    i.name
                    for i in sqlglot.parse_one(condition).find_all(
...
def remove_qualifier(query):
    return (
        ...
        .transform(
            lambda node: sqlglot.exp.column(node.this)
            if isinstance(node, sqlglot.exp.Column)
            else node
        )
        .sql()
...
def preprocess_conditions(events):
    ...
    for i in range(len(events)):
        ...
        if i != 0:
            assert (
                event_filter is not None and event_filter != sqlglot.exp.TRUE
            ), "only the first event can have no filter"

        assert event_name not in seen_events, "repeated event names not allowed"
        if event_filter is not None:
            event_filter = sqlglot.parse_one(event_filter)
            conjuncts = list(
                event_filter.flatten()
                if isinstance(event_filter, sqlglot.exp.And)
                else [event_filter]
            )
            for conjunct in conjuncts:
                ...
                assert (
                    "" not in conjunct_dependencies
                ), "must specify the table name in columns, like a.x instead of x"
                assert conjunct_dependencies.issubset(
                    seen_events.union({event_name})
                ), "an event can only depend on itself or prior events"

                if len(conjunct_dependencies) == 0:
                    raise Exception
                elif (
                    len(conjunct_dependencies) == 1
                    and list(conjunct_dependencies)[0] == event_name
                ):
                    event_prefilter = sqlglot.exp.and_(event_prefilter, conjunct)
                else:
                    event_dependent_filter = sqlglot.exp.and_(
                        event_dependent_filter, conjunct
                    )

                    if (
                        len(conjunct_dependencies) == 1
                        and list(conjunct_dependencies)[0] == events[0][0]
                    ):
                        if i == len(events) - 1:
                            last_event_dependent_on_first_event_filter = (
                                sqlglot.exp.and_(
                ...
                for i in all_columns:
                    if i.table != event_name:
                        event_required_columns[i.table] = event_required_columns.get(
                            i.table, set()
                        ).union({i.name})

        event_prefilter = optimizer.simplify.simplify(event_prefilter)
        event_prefilters[event_name] = event_prefilter.sql()
        event_dependent_filters[event_name] = optimizer.simplify.simplify(
            event_dependent_filter
        ).sql()
    ...
    last_event_dependent_on_first_event_filter = optimizer.simplify.simplify(
        last_event_dependent_on_first_event_filter
    ).sql()
    # print(prefilter)
...
def preprocess_2(
    ...
    if by is None:
        ...
        if type(by) == str:
            by = [by]
        assert all(
            [col in batch.columns for col in by]
        ), "groupby columns must be in batch"
        batch = batch.sort(by)
    ...
    for udf in udfs:
        if udf.lower() in udf_set:
            raise Exception(
                "UDF names are case insensitive. You cannot define both LIN_REG and lin_reg for example."
            )
    ...
    for event in event_predicates:
        ...
        for udf in my_udfs:
            ...
            for col in udf.expressions:
                assert (
                    type(col) == sqlglot.exp.Column
                ), "only non-modified column arguments are supported, i.e. you cannot do udf(x + 1)"
                col_event = col.table
                col_name = col.name
                if col_event == event:
            ...
            event_udfs[event].append(
                (name.upper(), udf_func, current_arguments, prior_arguments)
            )

        event_predicates[event] = (
            sqlglot.parse_one(predicate)
            .transform(
                lambda node: sqlglot.parse_one(node.this)
    ...
    event_required_columns = {
        event: ["__row_count__"] + list(event_required_columns[event])
        for event in event_names
    }

    select_cols = (
        touched_columns.union({time_col})
        if by is None
        else touched_columns.union({time_col, *by})
    )
    ...
    batch = (
        polars.SQLContext(frame=batch)
        .execute(
            "select __original_row_count__, {} from frame where {}".format(
                ",".join(select_cols), prefilter
            )
        )
        .collect()
    )
    ...
    event_independent_columns = {
        event: [
            k.name
            for k in sqlglot.parse_one(predicate).find_all(sqlglot.exp.Column)
            if k.table == event
        ]
        for event, predicate in events
        if predicate is not None
    }

    event_rename_dicts = {
        event: {
            k.sql(): k.table + "_" + k.name
            for k in sqlglot.parse_one(predicate).find_all(sqlglot.exp.Column)
        }
        for event, predicate in events
        if predicate is not None
    ...
    event_predicates = [
        replace_with_dict(predicate, event_rename_dicts[event])
        if (predicate is not None)
        else None
    ...
    rename_dicts = {
        event: {col: event + "_" + col for col in batch.columns}
        for event in event_names
    }

    assert (event_predicates[0] == "TRUE" or event_predicates[0] == None) and all(
        [predicate is not None for predicate in event_predicates[1:]]
    ...
    for event_name in event_names:
        if event_prefilters[event_name] != "TRUE":
            if by is None:
                event_indices[event_name] = (
                    polars.SQLContext()
                    .register(event_name, batch)
                    .execute(
                        "select __row_count__, {} from {} where {}".format(
                            time_col, event_name, event_prefilters[event_name]
                        )
                    )
                    .collect()
                )
        ...
        for event_name in event_names[1:]:
            if event_indices[event_name] is not None:
                ...
                event_indices[event_names[0]] = (
                    ...
                    .filter(
                        polars.col(time_col + "_right") - polars.col(time_col)
                        <= max_span
                    )
                    .select(
                        ["__row_count__", time_col] + (by if by is not None else [])
                    )
                )
...
def process_matched_events(
    ...
    if len(matched_events) > 0:
        ...
        matched_events = (
            matched_events.join(row_count_mapping, on="__row_count__", how="left")
            .drop(["__row_count__"])
            .rename({"__original_row_count__": "__row_count__"})
        )
