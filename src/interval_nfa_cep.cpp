...
extern "C" {
Vector2D MyFunction(PyObject *obj1, PyObject *obj2, KeyStringListPair *obj3,
                    KeyStringListPair *obj4, const char **obj5, DictEntry *obj6,
                    int num_events, int num_indices, const char *time_col) {
  ...
  std::map<std::string, std::vector<std::string>> event_independent_columns =
      processDict(obj4, num_events);
  std::map<std::string, std::unordered_set<int>> event_indices =
      processDictSet(obj6, num_indices);
  ...
  for (int event = 0; event < event_names.size(); event++) {
    ...
    for (int i = 0; i < event_independent_columns[event_name].size(); i++) {
      pos.push_back(
          schema->GetFieldIndex(event_independent_columns[event_name][i]));
      this_types.push_back(
          schema->GetFieldByName(event_independent_columns[event_name][i])
              ->type());
    }

    for (int i = 0; i < event_required_columns[event_name].size(); i++) {
      pos2.push_back(
          schema->GetFieldIndex(event_required_columns[event_name][i]));
    }
  ...
  for (int event = 0; event < event_names.size() - 1; event++) {
    ...
    for (int i = 0; i < event_required_columns[event_name].size(); i++) {
      current_cols.push_back(event_name + "_" +
                             event_required_columns[event_name][i]);
      types.push_back(
          schema->GetFieldByName(event_required_columns[event_name][i])
              ->type());
    }

    offsets.push_back(offsets.back() + current_cols.size());
    std::string sql =
        "CREATE TABLE matched_sequences_" + std::to_string(event) + " (";
  ...
  for (int event = 0; event < event_names.size(); event++) {
    if (event < event_names.size() - 1) {
      std::string sql = "INSERT INTO matched_sequences_" +
                        std::to_string(event) + " values (";
    ...
    if (event > 0) {
      ...
      std::string sql = "SELECT * FROM matched_sequences_" +
                        std::to_string(event - 1) + " WHERE " + predicate + ";";
      // std::cout << sql << std::endl;
  ...
  for (int event = 0; event < event_names.size() - 1; event++) {
    std::string sql =
        "DELETE FROM matched_sequences_" + std::to_string(event) + ";";
  ...
  for (size_t row = 0; row < batch->num_rows(); row++) {
    ...
    for (int event = 1; event < event_names.size(); event++) {
      if (event_indices.find(event_names[event]) == event_indices.end() ||
          event_indices[event_names[event]].find(row) !=
              event_indices[event_names[event]].end()) {
        this_row_can_be.push_back(event);
      }
    }
    // reverse sort this_row_can_be
    std::sort(this_row_can_be.begin(), this_row_can_be.end(),
              std::greater<int>());
  ...
  for (std::tuple<size_t, size_t> &start_end_pair : start_end) {
    ...
    for (size_t row = 1; row < num_rows; row++) {
      ...
      Scalar global_row_count_scalar =
          transposed_batch[row + start][row_count_idx_in_batch];
      ...
      for (int seq_len : rows_can_be[global_row_count]) {
        ...
        // bind_row_to_sqlite(db, filter_stmts[seq_len - 1], batch, row,
        // event_independent_columns[event_names[seq_len]]);

        for (int i = 0;
             i < event_independent_columns[event_names[seq_len]].size(); i++) {
        ...
        while (sqlite3_step(filter_stmts[seq_len - 1]) == SQLITE_ROW) {
          ...
          for (int col = 0;
            ...
            Scalar value = recover_scalar_from_stmt(filter_stmts[seq_len - 1],
                                                    col, types[col]);
            row.push_back(value);
          }
          ...
          if (seq_len == event_names.size() - 1) {
            ...
            for (int i = 0; i < row_count_idx.size(); i++) {
              ...
              if (std::holds_alternative<int>(row_count)) {
                ...
                std::cout << "error: row count type not understood"
                          << std::endl;
                throw 0;
              }
            }
            ...
            for (std::vector<Scalar> &matched_row : matched) {
              ...
              for (int pos : event_required_column_pos[event_names[seq_len]]) {
                bind_scalar_to_stmt(insert_stmts[seq_len], ++j,
                                    transposed_batch[global_row_count][pos]);
              }
  ...
  std::cout << "Insert elapsed time: " << insert_time.count() << " s\n";
